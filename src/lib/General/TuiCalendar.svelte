<script lang="ts">
  ...
  import type { ITheme } from "tui-calendar";
    ...
    calendarInstance = new Calendar(calendarElement, {
      ...
      template: {
        monthDayname: (dayname) => {
          return (
            '<span class="calendar-week-dayname-name bg-red-400">' +
            dayname.day +
        ...
        weekDayname: (dayName) => {
          return `<div class="calendar-week-dayname-name text-center flex flex-col">
                ${dayName.dayName} <br/>
                ${dayName.renderDate}
            </div>`;
        },
